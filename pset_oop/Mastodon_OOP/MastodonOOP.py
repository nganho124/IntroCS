...
class VideoMediaTrigger(MediaTrigger):
    def evaluate(self, toot):
        media = toot.media
        for content in media:
            # hier TypeError: string indices must be integers, not 'str', mit [1] hat es gelaufen, aber falsches ergebnis rausgekommen
...
class TimeTrigger(Trigger):
    def __init__(self, ptime):
        # convert triggered time -> assign timezone (-5 hours diff) -> replace
        dt = datetime.strptime(ptime, "%Y-%m-%d %H:%M:%S%z")
        est = timezone(timedelta(hours=-5))
        self.ptime = dt.replace(tzinfo=est)
...
class BeforeTrigger(TimeTrigger):
    def evaluate(self, toot):
        ...
        pubdate = toot.pubdate
        # Convert to EST
        if pubdate.tzinfo is None:
            pubdate = pytz.utc.localize(pubdate)
        # Strictly before
        return pubdate < self.ptime
